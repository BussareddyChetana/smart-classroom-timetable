...
def allocate_lab(subject, count, assigned_table, teacher_assignments, room_assignments, subject_slots_per_day, faculty):
    ...
    while placed < sessions_needed:
        ...
        place_periods(subject, faculty or 'Lab Faculty', room, day, periods, 'Lab', assigned_table, teacher_assignments, room_assignments, subject_slots_per_day)
        placed += 1
    return True


def allocate_crt(count, assigned_table, teacher_assignments, room_assignments, subject_slots_per_day, faculty):
    """Allocate CRT - 5 periods in 2+3 pattern on different days"""
    if count != 5:
        return False
    import random
    ...
    triplet_patterns = [('P1', 'P2', 'P3'), ('P2', 'P3', 'P4'), ('P3', 'P4', 'P5'), ('P4', 'P5', 'P6')]
    for pair_day, pair_periods, pair_room in pairs:
        ...
        place_periods('CRT', faculty or 'CRT Faculty', pair_room, pair_day, pair_periods, 'Theory', assigned_table, teacher_assignments, room_assignments, subject_slots_per_day)
        ...
        for day in days:
            ...
            for periods in patterns:
                ...
                if room and can_place(faculty, room, day, periods, assigned_table, teacher_assignments, room_assignments):
                    if all((day, period) not in room_assignments.get(room, set()) for period in periods):
                        place_periods('CRT', faculty or 'CRT Faculty', room, day, periods, 'Theory', assigned_table, teacher_assignments, room_assignments, subject_slots_per_day)
                        return True
...
def teacher_conflict(faculty, day, period, teacher_assignments):
    ...
    if not faculty:
        return False
    return (day, period) in teacher_assignments.get(faculty, [])
...
def view_timetable(section_id):
    ...
    for entry in timetable_entries:
        timetable[entry['day']][entry['period']] = entry
    section = query('SELECT * FROM sections WHERE section_id = %s', (section_id,), fetchone=True)
    return render_template('view_timetable.html', timetable=timetable, days=DAYS, periods=ACADEMIC_PERIODS, section=section)
