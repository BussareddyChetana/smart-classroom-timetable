...
THEORY_ROOMS = ['B-402', 'B-403', 'B-404']
...
def build_timetable_grid(entries):
    timetable = {day: {period: None for period in ACADEMIC_PERIODS} for day in DAYS}
...
def current_and_next_class(entries, today_name):
    ...
    today_entries = sorted(
        [entry for entry in entries if entry.get('day') == today_name],
        key=lambda item: ACADEMIC_PERIODS.index(item['period']) if item.get('period') in ACADEMIC_PERIODS else 99
    )
    ...
    for entry in today_entries:
        start, end = PERIOD_TIMES.get(entry.get('period'), (None, None))
    ...
    if next_class is None:
        today_index = DAY_INDEX.get(today_name, -1)
        upcoming = sorted(
            [
                entry for entry in entries
                if DAY_INDEX.get(entry.get('day'), 99) > today_index
            ],
            key=lambda item: (
                DAY_INDEX.get(item.get('day'), 99),
                ACADEMIC_PERIODS.index(item['period']) if item.get('period') in ACADEMIC_PERIODS else 99
            )
        )
...
def student_dashboard():
    ...
    timetable_entries = query(
        'SELECT * FROM timetable WHERE section_id = %s ORDER BY day, period',
        (student['section_id'],),
        fetchall=True
    )
...
def view_teachers():
    ...
    teachers = query('SELECT * FROM teachers ORDER BY teacher_name', fetchall=True)
...
def add_teacher():
    ...
    if request.method == 'POST':
        ...
        query(
            'INSERT INTO teachers (employee_id, teacher_name, department, specialization, password, max_hours_per_week, availability_status) VALUES (%s, %s, %s, %s, %s, %s, %s)',
            (employee_id, teacher_name, department, specialization, password, max_hours, availability_status),
            commit=True
        )
...
def edit_teacher(teacher_id):
    ...
    if request.method == 'POST':
        ...
        if request.form.get('password'):
            password = generate_password_hash(request.form['password'])
            query(
                'UPDATE teachers SET employee_id = %s, teacher_name = %s, department = %s, specialization = %s, password = %s, max_hours_per_week = %s, availability_status = %s WHERE teacher_id = %s',
                (employee_id, teacher_name, department, specialization, password, max_hours, availability_status, teacher_id),
                commit=True
            )
        else:
            query(
                'UPDATE teachers SET employee_id = %s, teacher_name = %s, department = %s, specialization = %s, max_hours_per_week = %s, availability_status = %s WHERE teacher_id = %s',
                (employee_id, teacher_name, department, specialization, max_hours, availability_status, teacher_id),
                commit=True
            )
        return redirect(url_for('view_teachers'))
...
def add_student():
    ...
    if request.method == 'POST':
        ...
        query(
            'INSERT INTO students (roll_number, password, section_id) VALUES (%s, %s, %s)',
            (roll_number, password, section_id),
            commit=True
        )
...
def edit_student(roll_number):
    ...
    if request.method == 'POST':
        ...
        if request.form.get('password'):
            password = generate_password_hash(request.form['password'])
            query(
                'UPDATE students SET password = %s, section_id = %s WHERE roll_number = %s',
                (password, section_id, roll_number),
                commit=True
            )
        else:
            query(
                'UPDATE students SET section_id = %s WHERE roll_number = %s',
...
def add_room():
    ...
    if request.method == 'POST':
        ...
        query(
            'INSERT INTO rooms (room_no, building, floor_no, capacity, room_type, equipment, is_active) VALUES (%s, %s, %s, %s, %s, %s, %s)',
            (room_no, building, floor_no, capacity, room_type, equipment, is_active),
            commit=True
        )
...
def edit_room(room_id):
    ...
    if request.method == 'POST':
        ...
        is_active = request.form.get('is_active') == 'on'
        query(
            'UPDATE rooms SET room_no = %s, building = %s, floor_no = %s, capacity = %s, room_type = %s, equipment = %s, is_active = %s WHERE room_id = %s',
            (room_no, building, floor_no, capacity, room_type, equipment, is_active, room_id),
            commit=True
        )
        return redirect(url_for('view_rooms'))
...
def view_subjects():
    ...
    subjects = query('SELECT * FROM subjects ORDER BY subject_name', fetchall=True)
...
def add_subject():
    ...
    if request.method == 'POST':
        ...
        query(
            'INSERT INTO subjects (subject_name, subject_type, faculty_name, semester, hours_per_week) VALUES (%s, %s, %s, %s, %s)',
            (subject_name, subject_type, faculty_name, semester, hours_per_week),
            commit=True
        )
...
def edit_subject(subject_id):
    ...
    if request.method == 'POST':
        ...
        hours_per_week = int(request.form['hours_per_week'])
        query(
            'UPDATE subjects SET subject_name = %s, subject_type = %s, faculty_name = %s, semester = %s, hours_per_week = %s WHERE subject_id = %s',
            (subject_name, subject_type, faculty_name, semester, hours_per_week, subject_id),
            commit=True
        )
        return redirect(url_for('view_subjects'))
...
def populate_timetable_for_section(section_id):
    ...
    subjects = query('SELECT * FROM subjects WHERE semester = %s ORDER BY subject_name', ('SEM-VI',), fetchall=True)
    ...
    subject_teachers = {sub['subject_name']: sub['faculty_name'] for sub in subjects}
    ...
    for _ in range(250):
        ...
        if valid and 'CRT' in remaining_counts:
            faculty = subject_teachers.get('CRT', 'CRT Faculty')
            if allocate_crt(remaining_counts['CRT'], trial_table, teacher_assignments, room_assignments, subject_slots_per_day, faculty):
        ...
        if valid and saturday_has_class and all_slots_filled:
    ...
    for day in DAYS:
        for period in ACADEMIC_PERIODS:
            ...
            if cell:
                query(
                    'INSERT INTO timetable (section_id, day, period, subject_name, faculty_name, room_no, subject_type) VALUES (%s, %s, %s, %s, %s, %s, %s)',
                    (section_id, day, period, cell['subject_name'], cell['faculty_name'], cell['room_no'], cell.get('subject_type', 'Theory')),
                    commit=True
                )
...
def place_periods(subject, faculty, room, day, periods, subject_type, assigned_table, teacher_assignments, room_assignments, subject_slots_per_day):
    ...
    for period in periods:
        ...
        teacher_assignments.setdefault(faculty_name, set()).add((day, period))
        room_assignments.setdefault(room, set()).add((day, period))
...
def allocate_crt(count, assigned_table, teacher_assignments, room_assignments, subject_slots_per_day, faculty):
    if count != 5:
        return False
    ...
    for day in DAYS:
        for periods in LAB_CONSECUTIVE:
            room = choose_room(THEORY_ROOMS, room_assignments, day, periods[0])

            if room and can_place(faculty, room, day, periods, assigned_table, teacher_assignments, room_assignments):
                if all((day, period) not in room_assignments.get(room, set()) for period in periods):
                    pairs.append((day, periods, room))
    ...
    for pair_day, pair_periods, pair_room in pairs:
        snapshot_table = {day: assigned_table[day].copy() for day in DAYS}
        snapshot_teachers = {name: slots.copy() for name, slots in teacher_assignments.items()}
        snapshot_rooms = {room: slots.copy() for room, slots in room_assignments.items()}
        snapshot_subject_days = {day: counts.copy() for day, counts in subject_slots_per_day.items()}

        place_periods(
            'CRT',
            faculty or 'CRT Faculty',
            pair_room,
            pair_day,
            pair_periods,
            'Theory',
        ...
        days = [day for day in DAYS if day != pair_day]
        random.shuffle(days)

        for day in days:
            patterns = list(triplet_patterns)
            random.shuffle(patterns)

            for periods in patterns:
                room = choose_room(THEORY_ROOMS, room_assignments, day, periods[0])
        ...
        assigned_table.update({day: snapshot_table[day] for day in DAYS})
        ...
        subject_slots_per_day.update(snapshot_subject_days)

    return False


def teacher_conflict(faculty, day, period, teacher_assignments):
    if not faculty:
        return False
    return (day, period) in teacher_assignments.get(faculty, set())


def room_conflict(room_assignments, room, day, period):
    return (day, period) in room_assignments.get(room, set())
